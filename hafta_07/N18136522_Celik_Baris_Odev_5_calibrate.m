function clbhw = N18136522_Celik_Baris_Odev_5_calibrate()
    clc, clear

    % Veriyi okuma s?ras?na göre kalibrasyon de?erlerini gir.
    S = [ 2.5 190; % 4.
        2.5 130; % 3.
        2.5 60; % 2.
        2.5 30; % 1.
    ...
    % Butterworth süzgeç parametrelerini belirle. 3 ve 4 pivot kullanmak
    % veriyi çok bozuyor ve 3 cü 4 cü dereceden paraboliklere benzetiyor.
    % Bu yüzden kullanmad?m.
    [b,a]= butter(2,6/500,'low');
    HF = filtfilt(b,a,H);
    
    % FFT test
    % H = calculate_fft(H, 1000);
    ...
    VF = zeros(numel(HF(:,1)), 2);
    for j = 2:numel(HF(:,1))-1
        VF(j,1) = (sqrt(sum((HF(j+1,:) - HF(j-1,:)) .^ 2))) / (2 * deltaT);
        VF(j,2) = j;
    end
    
    AF = zeros(numel(VF(:,1)), 2);
    for j = 2:numel(VF(:,2))-1
        AF(j,1) = (VF(j+1,1) - VF(j-1,1)) / (2 * deltaT);
        AF(j,2) = j;
    end
    
    figure;
    subplot(2,3,1)
    plot(V(:,2),H(:,2), 'm-s');grid on;
    title('Displacement - WeightedCentroid');
    xlabel('Time (frame)');
    ylabel('Displacement (cm)');
    subplot(2,3,2)
    plot(V(:,2),V(:,1)*(-1), 'm-s');grid on;%axis([0 1000 -0.1 0.7]);
    title('Velocity - WeightedCentroid');
    xlabel('Time (frame)');
    ylabel('Velocity');
    subplot(2,3,3)
    plot(A(:,2),A(:,1), 'm-s');grid on;axis([0 700 -0.06 0.06]);
    title('Acceleration - WeightedCentroid');
    xlabel('Time (frame)');
    ylabel('Acceleration');
    subplot(2,3,4)
    plot(VF(:,2),HF(:,2), 'b-s');grid on;%axis([0 1000 -0.055, 0.055]);
    title('Displacement - Butterworth Filtered');
    xlabel('Time (frame)');
    ylabel('Displacement (cm)');
    subplot(2,3,5)
    plot(VF(:,2),VF(:,1)*(-1), 'b-s');grid on;%axis([0 1000 -0.1 0.7]);
    title('Velocity - Butterworth Filtered');
    xlabel('Time (frame)');
    ylabel('Velocity');
    subplot(2,3,6)
    plot(AF(:,2),AF(:,1), 'b-s');grid on;axis([0 700 -0.0002 0.002]); %*2*(numel(HF(:,1))/1000)
    title('Acceleration - Butterworth Filtered');
    xlabel('Time (frame)');
    ylabel('Acceleration');
...
function Y = calculate_fft(rawdata, freq)
    ...
    %pp1 = splinefit(rawdata,Y,f);
    % Plot single-sided amplitude spectrum.
    figure, plot(f,2*abs(Y(1:NFFT/2)),'k:'),grid on;
    title(['Amplitude Spectrum of RawData'])
    xlabel('Frequency (Hz)')
    ylabel('|Y(f)|');
end


%% splinefit function import
